  ...
  return (
    <div className="min-h-screen bg-neutral-light">
      ...
      <ScrollAnimation className="relative py-20 md:py-32 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-neutral-light opacity-20" />
        ...
        >
          <motion.h1 className="text-4xl md:text-6xl font-bold mb-6 text-white" variants={textVariant(0.2)}>
            About CodeQuity
          </motion.h1>
          <motion.p className="text-xl md:text-2xl max-w-3xl mx-auto text-white/90" variants={textVariant(0.4)}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <p className="text-lg text-neutral-gray mb-6">
                Founded in 2020, CodeQuity emerged from a simple belief: that technology should be accessible to everyone. What started as a small group of passionate developers has grown into a global community of innovators, creators, and problem-solvers.
              </p>
              <p className="text-lg text-neutral-gray mb-8">
                We've built our platform on the principles of inclusivity, collaboration, and continuous learning. Every day, we work to break down barriers and create opportunities for developers of all skill levels to connect, learn, and grow together.
              </p>
              <Button className="bg-primary hover:bg-primary-hover text-white">Learn More</Button>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-6">
                {values.map((value, index) => (
                  <motion.div key={index} className="flex items-start" whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }}>
                    <div className="flex-shrink-0 mt-1 text-2xl">{value.icon}</div>
                    <div className="ml-4">
      ...
      <ScrollAnimation className="py-16 md:py-24 bg-neutral-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-16" initial="hidden" whileInView="show" viewport={{ once: true }} variants={staggerContainer(0.1, 0.2)}>
            <motion.h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4" variants={textVariant(0.2)}>
              Meet Our Team
            </motion.h2>
            <motion.p className="text-xl text-neutral-gray max-w-2xl mx-auto" variants={textVariant(0.4)}>
              The passionate individuals behind CodeQuity who are dedicated to building the future of developer communities.
            </motion.p>
          </motion.div>
                  ...
                  <CardContent className="p-6 text-center">
                    ...
                    <div className="flex justify-center space-x-4">
                      {Object.entries(member.social).map(([platform, url]) => {
                        if (!url || url === '#' || !iconMap[platform]) return null;
                        const Icon = iconMap[platform];
                        return (
                          <a key={platform} href={url} target="_blank" rel="noopener noreferrer" className="text-neutral-gray hover:text-primary transition-colors duration-200" aria-label={`${member.name}'s ${platform}`}>
      ...
      {/* The Timeline and Testimonials sections remain unchanged (unless you want improvements there too) */}
    </div>
  );
}
